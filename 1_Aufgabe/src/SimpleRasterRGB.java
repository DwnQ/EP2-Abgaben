...
public class SimpleRasterRGB {
    ...
    private int width;
    private int height;
    private SimpleDataBufferInt dataBufferInt;
    ...
    public SimpleRasterRGB convolve(double[][] filterKernel) {
        ...
        for (int x = filterSideLength; x < width - filterSideLength; x++) {
            for (int y = filterSideLength; y < height - filterSideLength; y++) {
                ...
                dataBufferInt.setElem(3, y * width + x, (int) temp_result[0]);
                dataBufferInt.setElem(4, y * width + x, (int) temp_result[1]);
                dataBufferInt.setElem(5, y * width + x, (int) temp_result[2]);
            }
        }
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < width*height; j++) {
                dataBufferInt.setElem(i,j, dataBufferInt.getElem(i+3,j));
            }
        }
        return this;
    }
